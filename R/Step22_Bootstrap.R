...
#   Rscript R/Step22_Bootstrap.R
#   Rscript run_step.R 22
# Sampling, fitting, prediction, summaries and plotting below follow the source.

if (!identical(environment(), globalenv())) {
  source("R/Step22_Bootstrap.R", local = globalenv(), encoding = "UTF-8")
} else {
  source("R/lib/oef_utils.R")
  oef_start_step("Step22")
...
InFile <- oef_input("trust_lifespan_hc.csv")
OutDir <- oef_output("sensitivity_bootstrap")
