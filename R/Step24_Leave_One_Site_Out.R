...
Display_name <- "OEF"
output_dir <- oef_output("sensitivity_loso")
...
fit_and_predict_loso <- function(train_data, age_grid) {
  ...
  pred_m <- tryCatch({
    oef_predict_all(random = "zero", mod, newdata = newdata_m)
  }, error = function(e) {
  ...
  pred_f <- tryCatch({
    oef_predict_all(random = "zero", mod, newdata = newdata_f)
  }, error = function(e) {
