...
OutDir         <- oef_output("lifespan_model") 
OutDir2        <- oef_output("lifespan_model/data") 
...
axis(1, at = seq_along(sort(unique(M_HC$SiteID))), labels = sort(unique(M_HC$SiteID)), las = 2, cex.axis = AxisCex*0.7)
...
models <- lapply(dfs, function(df_mu) {
  tryCatch(oef_fit_gamlss(
    phenotype ~ bs(Age, df = df_mu) * Sex + random(as.factor(SiteID)),
    sigma.fo = ~ bs(Age, df = best_df_sigma) + Sex,
    nu.fo = ~ 1, tau.fo = ~ 1, family = BCTo,
    data = M_HC, control = con
  ), error = function(e) {
    message("Candidate df=", df_mu, " failed: ", conditionMessage(e))
    NULL
  })
})
converged <- vapply(models, function(m) !is.null(m) && isTRUE(m$converged), logical(1))
bic_vals <- vapply(models, function(m) if (is.null(m)) NA_real_ else as.numeric(m$sbc), numeric(1))
valid <- which(converged & is.finite(bic_vals))
if (!length(valid)) stop("No finite, converged candidate model is available.")
...
make_centiles <- function(sex_val, model, age_grid, q_vec, site_levels) {
  ...
  pa <- oef_predict_all(random = "zero", model, newdata = nd)
...
y_lim <- range(c(0, y_all), na.rm = TRUE)
...
cat("\n========== 10-fold cross-validation z-scores =========\n")
set.seed(123)
...
for (k in 1:10) {
  ...
  pa <- tryCatch(
    oef_predict_all(random = "zero", cv_model, newdata = test_dat),
    error = function(e){
