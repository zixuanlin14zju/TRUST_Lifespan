# Step21_Balanced_Resampling.R
# Start in the repository root. Either of these entry points is supported:
#   Rscript R/Step21_Balanced_Resampling.R
#   Rscript run_step.R 21
# Sampling, fitting, prediction, summaries and plotting below follow the source.

if (!identical(environment(), globalenv())) {
  source("R/Step21_Balanced_Resampling.R", local = globalenv(), encoding = "UTF-8")
} else {

  source("R/lib/oef_utils.R")
  oef_start_step("Step21")
...
M_HC <- read.csv(oef_input("trust_lifespan_hc.csv"))
Phenotype_name <- "OEF" 
...
OutDir <- oef_output("sensitivity_balanced_resampling") 
...
for (i in 1:n_resample) {
  ...
  mod <- gamlss(
    phenotype ~ bs(Age, df=3) * Sex + random(SiteID),
    sigma.fo = ~ bs(Age, df=3) + Sex,
    nu.fo = ~ 1,
    tau.fo = ~ 1,
    family = BCTo,
    data = tmp_data,
