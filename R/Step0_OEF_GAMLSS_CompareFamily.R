# Step0_OEF_GAMLSS_CompareFamily.R
# Public analysis entry point. Start from the repository root, or use:
#   Rscript run_step.R 0
source("R/lib/oef_utils.R")
oef_start_step("Step0")

# ============================================================
# Compare GAMLSS family + df_mu + df_sigma
# ============================================================
...
# 01) User settings
# ============================================================
DataPath <- oef_input("trust_lifespan_hc.csv")
OutDir2  <- oef_output("model_selection")
...
fit_gamlss_safe <- function(data, df_mu, df_sigma, fam_fun, control_obj) {
  out <- tryCatch(
    oef_fit_gamlss(
      phenotype ~ bs(Age, df = df_mu, degree = 3) * Sex + random(as.factor(SiteID)),
      sigma.fo  = ~ bs(Age, df = df_sigma, degree = 3) + Sex,
      nu.fo     = ~ 1,
      tau.fo    = ~ 1,
      family    = fam_fun,
      data      = data,
...
cat("Unique cubic B-spline grid: df_mu =", paste(dfs_mu, collapse = ", "),
    "| df_sigma =", paste(dfs_sigma, collapse = ", "), "\n")
...
results_ranked_bic <- results_df %>%
  filter(Converged, is.finite(AIC), is.finite(BIC)) %>%
  arrange(BIC, AIC)
...
# ============================================================
# 07) Plot model comparison results
# ============================================================
...
# Keep only converged models for plotting
plot_df <- results_df %>%
  filter(Converged)
...
if (nrow(plot_df) == 0) {
  ...
  # ==========================================================
  # 07.3 Best BIC within each family
  # ==========================================================
  ...
  p_fam_bic <- ggplot(fam_best_bic, aes(x = Family, y = BIC, fill = Family)) +
    geom_col(color = "black", linewidth = 0.2) +
    geom_text(aes(label = Label), hjust = -0.05, size = 3.8, family = "sans") +
    coord_flip(clip = "off") +
    labs(
  ...
  # ==========================================================
  # 07.4 Best AIC within each family
  # ==========================================================
  ...
  # ==========================================================
  # 07.5 BIC heatmap for each family
  # ==========================================================
  for (fam in unique(plot_df$Family)) {
    ...
    p_heat_bic <- ggplot(fam_df, aes(x = factor(DF_Mu), y = factor(DF_Sigma), fill = BIC)) +
      ...
      geom_text(aes(label = round(BIC, 1)), size = 3.5, family = "sans") +
      labs(
  ...
  # ==========================================================
  # 07.6 AIC heatmap for each family
  # ==========================================================
  for (fam in unique(plot_df$Family)) {
    fam_df <- plot_df %>%
      filter(Family == fam)
    ...
    p_heat_aic <- ggplot(fam_df, aes(x = factor(DF_Mu), y = factor(DF_Sigma), fill = AIC)) +
      geom_tile(color = "white", linewidth = 0.5) +
      geom_text(aes(label = round(AIC, 1)), size = 3.5, family = "sans") +
      labs(
  ...
  # ==========================================================
  # 07.8 Family-wise summary table
  # ==========================================================
